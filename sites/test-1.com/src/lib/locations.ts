 ...
 */
import { areaSettings, listPublished, siteSettings, type Doc } from './runtime-content';

/** A city entry as it comes back from the runtime store. */
export type LocationEntry = Doc<any>;
...
const entrySlug = (entry: LocationEntry) => entry.slug;
...
export async function getCities(): Promise<CityRef[]> {
  const entries = await listPublished<any>('locations');
  return entries
...
export async function getAreaServices(): Promise<CityService[]> {
  const entries = await listPublished<any>('services');
  const bySlug = new Map(entries.map((entry) => [entry.slug, entry]));

  return (areaSettings().services as string[])
    .map((slug) => {
...
export function fillTokens(
  ...
  const all: Record<string, string | undefined> = {
    ...
    phone: siteSettings().business.contact.phone,
    ...tokens,
  };
  return template
...
// areaSettings now lives in runtime-content.ts, re-exported so the 27 call
// sites that read it keep working unchanged.
export { areaSettings } from './runtime-content';
