...
//
// `scripts/optimize-images.mjs` still guards the repo-backed half against an
// oversized upload; the R2 half is downscaled in the browser before it is sent.
type ImageOpts = { label: string; description?: string; required?: boolean };

/**
 * Photograph — goes through <Picture />, stored in R2.
 *
 * `r2Image` posts the file to /api/media the moment it is dropped and keeps
 * `r2:<key>` in the entry, so a page save is a one-line JSON diff. The build
 * fetches the original from the bucket and encodes the same AVIF/WebP ladder
 * sharp produced when these lived in src/assets — the rendered output is
